const plugin = {
  install (Vue, options) {
      ...
      Oss: {
        ...
        getCdnUrl: function (bucket, url) {
          if (this.startWithHttp(url)) {
            ...
            if (bucket.startsWith('gamecircle-')) {
              return 'http://' + bucket.replace(new RegExp(/(gamecircle-)/g), '') + '.youka.youmengchuangxiang.com/' + url;
            } else if (bucket.startsWith('hjdog-')) {
              return 'http://' + bucket.replace(new RegExp(/(hjdog-)/g), '') + '.v.hj-dog.com/' + url;
            }
            return '';
          }
        },
        getImageResizeCdnUrl: function(bucket, url, width, height, color) {
          return this.imageResize(this.getCdnUrl(bucket, url), width, height, color)
        },
        getVideoSnapshotCdnUrl: function(bucket, url, t, width, height) {
          return this.videoSnapshot(this.getCdnUrl(bucket, url), t, width, height)
        },
        //获取视频帧截图
        videoSnapshot: function(url, t, width, height) {
          return url + '?x-oss-process=video/snapshot,t_' + (t ? t : 0) + ',f_jpg,w_' + (width ? width : 0) + ',h_' + (height ? height : 0);
        },
        //固定宽度，自动居中裁剪
        ...
        //固定宽高，缩略填充
        imageResize: function (url, width, height, color) {
          return url + '?x-oss-process=image/resize,m_pad,h_' + height + ',w_' + width + (color ? (',color_' + color) : '');
        }
      },
      UserExt: {
    ...
    // endsWith只支持部分浏览器，这里重写一下这个函数让所有浏览器支持
    if (typeof String.prototype.endsWith != 'function') {
      String.prototype.endsWith = function (prefix){
        return this.slice(this.length - prefix.length) === prefix
      }
    }
